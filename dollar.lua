...
DollarRecognizer = function() -- constructor
    ...
    self.recognize = function(points, useProtractor)
        ...
        local name = self.Templates[t] and self.Templates[t].Name or ''
        local score = useProtractor and 1.0 / b or 1.0 - b / HalfDiagonal
        return name, score
    end

    -- returns number of recorded gestures with same name
    self.add = function(name, points)
        self.Templates[#self.Templates + 1] = Template(name, points) -- append new template
        local num = 0
...
ScaleTo = function(points, size) -- non-uniform scale assumes 2D gestures (i.e., no lines)
    ...
    local newpoints = {}
    for i = 1, #points, 1 do
        local qx = points[i][1] * (size / B.Width)
        local qy = points[i][2] * (size / B.Height)
        newpoints[#newpoints+1] = Point(qx, qy)
    end
    return newpoints
end
